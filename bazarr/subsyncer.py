...
class SubSyncer:
    ...
    def sync(self, video_path, srt_path, srt_lang, media_type, advanced_mode, sonarr_series_id=None, sonarr_episode_id=None,
             radarr_id=None,):
        self.reference = video_path
        self.srtin = srt_path
        self.srtout = '{}.synced.srt'.format(os.path.splitext(self.srtin)[0])
        self.embeddedSrtOut = '{}.extracted.srt'.format(os.path.splitext(self.srtin)[0])
        self.args = None
        ...
        try:
            ...
            if self.advanced or settings.subsync.getboolean('use_reftrack'):
                
                subtitleDict,syncMethod = subtools.getSubtitleByType(media_type)
                refSub = subtools.getReferenceSubtitle(subtitleDict, syncMethod)
                if not refSub:
                    return
                if len(refSub) <= 5:
                    self.reference = subtools.extractSubtitle(self.reference, refSub, self.embeddedSrtOut, ffmpeg_exe)
                    unparsed_args = subtools.getNewSubSyncArgs(self.reference, self.srtin, self.srtout, self.ffmpeg_path, self.vad, self.log_dir_path, syncMethod)
                else:
                    self.reference = refSub
                    unparsed_args = subtools.getNewSubSyncArgs(self.reference, self.srtin, self.srtout, self.ffmpeg_path, self.vad, self.log_dir_path, syncMethod)


            if settings.subsync.getboolean('debug'):
                unparsed_args.append('--make-test-case')
            if  settings.subsync.getboolean('use_offset'):
                unparsed_args.append('--apply-offset-seconds')
                unparsed_args.append(settings.subsync.use_offset_value)
            if settings.subsync.getboolean('use_gss'):
                unparsed_args.append('--gss')
            if settings.subsync.getboolean('use_maxoffset_override'):
                unparsed_args.append('--max-offset-seconds')
                unparsed_args.append(settings.subsync.use_maxoffset_value)
            if settings.subsync.getboolean('debug'):
                unparsed_args.append('--make-test-case')

            parser = make_parser()
            self.args = parser.parse_args(args=unparsed_args)
            logging.debug('Running with args: %s:', self.args)
            ...
            if settings.subsync.getboolean('debug'):
                return result
            if os.path.isfile(self.embeddedSrtOut):
                os.remove(self.subextractsrtout)     
